...
- **Проверка владельца на уровне запроса к БД** - `userId` из JWT добавляется прямо в фильтр `findOne`/`findOneAndUpdate`, поэтому чужой ресурс физически не находится (возврат `404`), а не проверяется отдельным шагом.
...
**Время выполнения:** _<укажи фактическое время, напр. ~12 часов>_
